...
====
VOX is not ideal for data. The VOX built in most radios will start transmitting slowly because VOX is designed to not transmit from short noise bursts. It requires using a long TXDELAY setting in the TNC, so that the transmit keys before actual packet data is transmitted.
